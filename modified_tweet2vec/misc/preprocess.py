...
with io.open(outfile, 'w') as tweet_processed_text, io.open(infile, 'r') as fin:
    ...
    # for line in fin:
    # 	res = preprocess(line.rstrip())
    #     if res == '':
    #           res = unicode('', "utf-8")
    #     tweet_processed_text.write(res+'\n')

    ####Pass Tweet+Emoji data instead of just tweets. If preprocess return '', don't append
    data = csv.reader(fin, delimiter=',', quotechar="|")
    for line in data:
